...
COMMENT ON COLUMN public.profiles.user_category IS 'Category of the user: curioso, aluno_usp, or pesquisador.';
COMMENT ON COLUMN public.profiles.seeking_ic IS 'Flag for students looking for Undergraduate Research (IC).';
COMMENT ON COLUMN public.profiles.seeking_assistant IS 'Flag for researchers looking for academic assistants.';
...
COMMENT ON TABLE public.micro_articles IS 'Stores micro-posts limited to 260 characters.';
...
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
-- Migration: Add status and featured columns to micro_articles
-- Description: Supports admin moderation and highlighting.

ALTER TABLE public.micro_articles 
ADD COLUMN IF NOT EXISTS status text DEFAULT 'pending',
ADD COLUMN IF NOT EXISTS is_featured boolean DEFAULT false;

-- Policy update: Admin can update/delete all, but public can only see approved
DROP POLICY IF EXISTS "Public can read micro-articles" ON public.micro_articles;
CREATE POLICY "Public can read micro-articles" ON public.micro_articles
    FOR SELECT USING (
        status = 'approved' OR 
        is_featured = true OR 
